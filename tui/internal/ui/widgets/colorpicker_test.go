...
// settleZone waits for a scanned zone to settle: the manager stores zones on a background worker.
...
// Eight columns puts the 16 ANSI colors in the first two rows, normal then bright.
...
// Horizontal steps run the palette linearly, a down step jumps a row, and both clamp at the edges.
...
// Seeding the cursor is case-insensitive.
