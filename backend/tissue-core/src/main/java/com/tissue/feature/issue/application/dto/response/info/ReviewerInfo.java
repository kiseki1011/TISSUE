...
public record ReviewerInfo(ProjectMemberInfo participant, ReviewStatus status) {
    public static ReviewerInfo from(IssueReviewer reviewer) {
        return new ReviewerInfo(ProjectMemberInfo.from(reviewer.getReviewer()), reviewer.getStatus());
    }
}
