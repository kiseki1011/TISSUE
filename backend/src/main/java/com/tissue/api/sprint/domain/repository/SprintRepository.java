...
import com.tissue.api.issue.presentation.condition.IssueSearchCondition;
...
public interface SprintRepository extends JpaRepository<Sprint, Long> {
	...
	// 	@Param("sprintKey") String sprintKey,
	// 	@Param("workspaceCode") String workspaceCode,
	// 	@Param("condition") IssueSearchCondition condition,
	// 	Pageable pageable
	// );
	...
	@Query("SELECT si.issue FROM Sprint s "
		...
		+ "AND (:#{#condition.types.isEmpty()} = true OR si.issue.type IN :#{#condition.types}) "
		+ "AND (:#{#condition.priorities.isEmpty()} = true OR si.issue.priority IN :#{#condition.priorities}) "
		+ "AND (:#{#condition.keyword} IS NULL OR "
		+ "LOWER(si.issue.issueKey) LIKE LOWER(CONCAT('%', :#{#condition.keyword}, '%')) OR "
		+ "LOWER(si.issue.title) LIKE LOWER(CONCAT('%', :#{#condition.keyword}, '%')) OR "
		+ "LOWER(si.issue.content) LIKE LOWER(CONCAT('%', :#{#condition.keyword}, '%')))")
	Page<Issue> findIssuesInSprint(
		@Param("sprintKey") String sprintKey,
		@Param("workspaceCode") String workspaceCode,
		@Param("condition") IssueSearchCondition condition,
