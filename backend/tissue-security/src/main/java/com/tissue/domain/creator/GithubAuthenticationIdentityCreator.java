...
@Component
public class GithubAuthenticationIdentityCreator implements AuthenticationIdentityCreator {

    @Override
    public boolean supports(AuthenticationProvider provider) {
        return provider == AuthenticationProvider.GITHUB;
    }

    @Override
    public AuthenticationIdentity create(Member member, String identifier, @Nullable String credential) {
        return AuthenticationIdentity.createSocialIdentity(member, AuthenticationProvider.GITHUB, identifier);
    }
}
